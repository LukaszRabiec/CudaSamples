// Lab6_MultiplyTwoMatrixesOnBlocks.cu : Defines the entry point for the console application.
...
__global__ void MultiplyMatrixes(const int* firstMatrix, const int* secondMatrix, int* resultMatrix)
{
	...
	if (row < blockDim.x && col < blockDim.y)
	{
...
int main()
{
	...
	MultiplyMatrixes<<<grid, block>>>(devFirstMatrix, devSecondMatrix, devMatrix);
